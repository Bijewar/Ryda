...
const FLOWS = [
  {
    ...
    title: "Realtime",
    body:
      "Passenger requests a ride → Socket.IO broadcasts to drivers in a 5 km PostGIS radius → first accept wins → live location streams over Redis pub/sub, not the DB.",
    accent: "#FF5722",
  },
  {
    icon: CreditCard,
    title: "Payments",
    body:
      "Idempotent Razorpay order creation (X-Idempotency-Key header) → HMAC-verified webhooks → atomic Payment state machine → Razorpay Route payouts to drivers. 100% free — no Stripe.",
    accent: "#2DD4BF",
  },
  {
    icon: MapIcon,
    title: "Geofence",
    body:
      "Bhopal polygon from OSM → PostGIS ST_Contains on every pickup / dropoff → driver-online toggle requires ST_Contains(driver, bhopal) → surge computed per-ward density.",
    accent: "#FF4081",
  },
] as const;
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
        ...
        >
          ...
          <p className="max-w-2xl text-base sm:text-lg text-ryda-muted leading-relaxed">
            One Next.js process serves the client, the API, and the edge
            middleware. Socket.IO and BullMQ run as separate processes so they
            scale independently. Postgres + PostGIS is the only source of
            truth.
          </p>
        </motion.div>
        ...
        <div className="relative">
          ...
          >
            ...
            <motion.line
              x1="50%" y1="0" x2="50%" y2="33%"
              stroke="url(#ryda-conn)" strokeWidth="1.6" strokeDasharray="4 4"
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <h3 className="text-sm sm:text-base font-bold text-ryda-text">
                      {box.title}
                    </h3>
                  </div>
                  <p className="text-xs sm:text-sm leading-relaxed text-ryda-muted">
                    {box.desc}
                  </p>
                </motion.div>
              );
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 mb-3">
                  ...
                  <h3 className="text-base font-bold text-ryda-text">
                    {flow.title}
                  </h3>
                  <span className="ml-auto text-xs font-mono text-ryda-muted">
                    0{i + 1}
                  </span>
                </div>
                <p className="text-sm leading-relaxed text-ryda-muted">
                  {flow.body}
                </p>
              </motion.div>
            );
